...
class SoFIFA(BaseRequestsReader):
    ...
    def read_team_ratings_nationals(self) -> pd.DataFrame:
        """Retrieve ratings for all national teams.
        ...
        # Build url for national teams
        list_int_forgotten_nations = [1, 20, 43] # 1: Albania, 20: Georgia, 43: Slovakia
        urlmask = SO_FIFA_API + "/teams?type=national&r={}&set=true"
        urlmask_forgotten_nations = SO_FIFA_API + "/teams?type=history&r={}&set=true"
        for country_id in list_int_forgotten_nations:
            urlmask_forgotten_nations += f"&na%5B%5D={country_id}"
        for rating_id in ratings.keys():
            urlmask += f"&showCol[]={rating_id}"
            urlmask_forgotten_nations += f"&showCol[]={rating_id}"
        filemask = "national_teams_{}.html"
        # Collect teams
        teams = []
        iterator = list(self.versions.iterrows())
        for i, (version_id, version) in enumerate(iterator):
            # logger(
            #     "[%s/%s] Retrieving ratings for national teams in %s edition",
            #     i + 1,
            #     len(iterator),
            #     version["update"],
            # )
            # Read html page (national teams overview)
            filepath = self.data_dir / filemask.format(version_id)
            for url_ in [urlmask, urlmask_forgotten_nations]:
                url = url_.format(version_id)
                reader = self.get(url, filepath)
    ...
    def read_team_ratings_by_name(self, team_names: List[str]) -> pd.DataFrame:
        """
        Retrieve ratings for specific teams by name (useful for non-Big Five or non-national teams).

        Parameters
        ----------
        team_names : list of str
            List of exact team names to look for using the `keyword` search on SoFIFA.

        Returns
        -------
        pd.DataFrame
            A dataframe containing ratings for the teams found by keyword search.
        """
        # Define the ratings to extract
        ratings = {
            "oa": "overall",
            "at": "attack",
            "md": "midfield",
        ...
        for team_name in team_names:
            for version_id, version in self.versions.iterrows():
                keyword = team_name.replace(" ", "%20")
                url = f"{SO_FIFA_API}/teams?keyword={keyword}&r={version_id}&set=true"
                for rating_id in ratings.keys():
                    url += f"&showCol[]={rating_id}"
                file_safe_name = re.sub(r"[^\w]+", "_", team_name.strip())
                filepath = self.data_dir / f"team_search_{file_safe_name}_{version_id}.html"
                logger.info(f"Searching for team '{team_name}' in version {version['update']}...")
                ...
                try:
                    ...
                    for row in tree.xpath("//table/tbody/tr"):
                        row_team_name = row.xpath(".//td[2]//a")[0].text.strip()
                        if row_team_name.lower() == team_name.lower():
                            # parse ratings
                            result = {
                                "team": row_team_name,
                                **{
                                    desc: row.xpath(f".//td[@data-col='{rid}']//text()")[0].strip()
                                    for rid, desc in ratings.items()
                                    if row.xpath(f".//td[@data-col='{rid}']//text()")
                                },
                                **version.to_dict(),
                            }
                            results.append(result)
                except Exception as e:
                    logger.warning(f"Failed to retrieve data for {team_name} (v{version_id}): {e}")
                    continue

        if not results:
            raise ValueError("No team ratings could be retrieved from the given list of names.")
