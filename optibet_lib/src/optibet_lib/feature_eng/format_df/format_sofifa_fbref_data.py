...
def format_sofifa_fbref_data(fbref_df_date_filtered_concat, datetime_stop=None,
    ...
    datetime_stop = datetime.datetime.now().replace(hour=0, minute=0, second=0, microsecond=0) if not datetime_stop else datetime_stop
    rule_is_null = (fbref_df_date_filtered_concat_no_nan[ftr_col].isnull())
    rule_is_future_match = (fbref_df_date_filtered_concat_no_nan[datetime_col] >= datetime_stop)
