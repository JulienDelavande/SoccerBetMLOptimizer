...
class SoFIFA(BaseRequestsReader):
    ...
    def read_national_teams(self) -> pd.DataFrame:
        url = SO_FIFA_API + "/teams?type=national"
        filepath = self.data_dir / "national_teams.html"
        reader = self.get(url, filepath)
        ...
        for node in tree.xpath("//table/tbody/tr"):
            ...
            teams.append(
                {
                    "team_id": int(re.search(pat_team, team_link.get("href")).group(1)),
